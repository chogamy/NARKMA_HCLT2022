...
class DecoderLayer(nn.Module):
    def __init__(self, args):
        ...
        self.decoder_attn_layer_norm = nn.LayerNorm(args.d_model)
        self.final_layer_norm = nn.LayerNorm(args.d_model)
...
class Decoder(nn.Module):
    def __init__(self, args, vocab_size, pad_ids, embed_tokens: Optional[nn.Embedding] = None):
        ...
        # self.dropout_morph = nn.Dropout(args.dropout)
        #self.dropout_tag = nn.Dropout(args.dropout)
        # self.layer_norm_morph = nn.LayerNorm(self.args.d_model)
        # self.layer_norm_tag = nn.LayerNorm(self.args.d_model)
...
class NATransformer(nn.Module):
    ...
    def forward(self, input_ids=None, attention_mask=None, 
                    morph_input_ids=None, morph_attention_mask=None, morph_labels=None, 
                    tag_input_ids = None, tag_attention_mask=None, tag_labels=None, 
                    len_labels=None
    ):
        enc_outputs, length = self.encoder(input_ids, attention_mask)
        # enc_outputs : [bsz, max_len -1, d_model]
        # attention_mask : [bsz, max_len]

        morph_dec_outputs, _ = self.morph_decoder(morph_input_ids, morph_attention_mask, 
                                                enc_outputs, attention_mask[:,1:])

        tag_dec_outputs, _ = self.tag_decoder(tag_input_ids, tag_attention_mask, 
                                                enc_outputs, attention_mask[:,1:])


        morph_attention_mask = _expand_mask(morph_attention_mask, morph_dec_outputs.dtype)
        tag_attention_mask = _expand_mask(tag_attention_mask, tag_dec_outputs.dtype)

        #first
        morph_dec_outputs, _ = self.morph_decoder.layers[0](
                morph_dec_outputs,
                tag_dec_outputs,
                morph_attention_mask,
                tag_attention_mask
        )
        ...
        return morph_logits, tag_logits, length, loss
